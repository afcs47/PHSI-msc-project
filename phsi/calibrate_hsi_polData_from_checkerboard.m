function tform = calibrate_hsi_polData_from_checkerboard(hsiCalibData, polCalibData, datasetName, resizeSizeHsi, resizeSizePol, check_calibration) %Aligns and fuses hsi and polarization images of checkerboard pattern via homography matrix

    %% Handle HSI input
    %hsi_data = hsiCalibData(1:resizeSizeHsi(1), 1:resizeSizeHsi(2), :); % Resize/crop to match reference
    hsi_data = imresize(hsiCalibData, [resizeSizeHsi(1), resizeSizeHsi(2)]);
    %size(hsiCalibData)
    %size(hsi_data)
    %resizeSizeHsi

    %% Handle POL input
    %pol_data = polCalibData(1:resizeSizePol(1), 1:resizeSizePol(2), :); % Resize/crop to match reference
    pol_data = imresize(polCalibData, [resizeSizePol(1), resizeSizePol(2)]);
    %size(pol_data)
    %resizeSizePol
    ...
    [imagePointsHSI, boardSizeHSI] = try_detect_checkerboard(hsi_data, 'HSI'); % Detect in HSI RGB
    [imagePointsPOL, boardSizePOL] = try_detect_checkerboard(pol_data, 'Polarization'); % Detect in Polarization RGB
    ...
    % HSI Image
    figure('Name','HSI Checkerboard'); imshow(hsi_data); axis image; % Ensure square pixels
    ...
    % Polarization Image
    figure('Name','Pol Checkerboard' ); imshow(pol_data); axis image; % Ensure square pixels
    ...
    % make sure the vertical misalignment issue doesn't come from the homography 
    ...
    [tform_best, tform_fit,tform_est, report] = compare_calibration_methods_ROI(ptsHSI, ptsPOL, imagePointsHSI, imagePointsPOL,hsi_data, pol_data, check_calibration);
    ...
    % Menu selection
    choice = menu(sprintf('Select transformation (recommended: %s based on RMSE)', names{bestIdx}), 'Use tform_fit','Use tform_est');
    % Assign chosen transform
    tform = eval(names{choice});  
    fprintf('Using %s.\n', names{choice});

    % Use the best transform for everything else:
    alignedPOL = imwarp(pol_data, tform, 'OutputView', imref2d(size(hsi_data)));

    %% Show and save fused result (tranform matrix) for future use
    figure('Name','Fused Checkerboards (falsecolor)' ); imshowpair(hsi_data, alignedPOL, 'falsecolor'); %Colored regions show where the intensities differ
    title('Fused Image: HSI + aligned POL');
    axis image; % Ensure square pixels
    
    save([datasetName '_homography_tform.mat'], 'tform');
    disp("✅ Calibration completed. Parameters saved to '*_homography_tform.mat'."); 

    if check_calibration
        disp('Checking callibration...')
        ...
        figure('Name','Fused Checkerboards (diff)' ); imshowpair(hsi_data,alignedPOL,"diff");
        ...
        %Normalization by maximum distance in the image -> not as meaningful for reprojection accuracy as the checkerboard may not be centered in origin (0,0)
        ...
        %Normalization of residuals, again distance from the origin may not correspond to actual scale
        ...
        fprintf("\n  Mean = %.4f px (%.4f %% - maxRadialDist)", mean(residualsFull), mean(residualsPercent));
        fprintf("\n  STD  = %.4f px", std(residualsFull));
        fprintf("\n  Max  = %.4f px\n", max(residualsFull));

        %Normalization of residuals but considering image diagonal length
        imgDiagleng = norm([size(hsi_data,2), size(hsi_data,1)]);
        residualsPercent = residualsFull / imgDiagleng * 100;
        fprintf("\n  Mean = %.4f px (%.4f %% - imgDiagleng)", mean(residualsFull), mean(residualsPercent));
