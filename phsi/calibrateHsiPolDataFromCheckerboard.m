function tform = calibrateHsiPolDataFromCheckerboard(hsiCalibData, polCalibData, datasetName, resizeSizeHsi, resizeSizePol, check_calibration) %Aligns and fuses hsi and polarization images of checkerboard pattern via homography matrix

    % Handle HSI input (DoLP or reflectance struct)
    ...
    [imagePointsHSI, boardSizeHSI] = tryDetectCheckerboard(hsi_data, 'HSI'); % Detect in HSI RGB
    [imagePointsPOL, boardSizePOL] = tryDetectCheckerboard(pol_data, 'Polarization'); % Detect in Polarization RGB
    ...
    % HSI Image
    figure; imshow(hsi_data); axis image; % Ensure square pixels
    ...
    % Polarization Image
    figure; imshow(pol_data); axis image; % Ensure square pixels
    ...
    [tform_best, tform_fit,tform_est, report] = compareCalibrationMethodsROI(ptsHSI, ptsPOL, imagePointsHSI, imagePointsPOL,hsi_data, pol_data, check_calibration);
    ...
    figure; imshowpair(hsi_data, alignedPOL, 'falsecolor'); %Colored regions show where the intensities differ
    ...
    if check_calibration
        ...
        figure; imshowpair(hsi_data,alignedPOL,"diff");
