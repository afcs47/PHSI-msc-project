function [tform_best, tform_fit, tform_est, report] = compare_calibration_methods_ROI(ptsHSI, ptsPOL, imagePointsHSI, imagePointsPOL, hsi_rgb, pol_rgb, check_calibration) % Compare fitgeotform2d (4 manual corners) vs estgeotform2d (matched ROI
...
% err_fit and err_est are pixel-space RMSEs between transformed POL points
% and corresponding HSI points -> Checks if the transform maps the specific
% matched points accurately (resulting in artificially small errors),
% however it does not measure the global alignment accuracy over the whole
% image, and since accuracy varies spatially for projective transforms,
% other metricss need to be applied
    ...
    %% Other metrics for better global comparison

    % Check reprojection error for all matched points
    [x_all_f, y_all_f] = transformPointsForward(tform_fit, roiPOL(:,1), roiPOL(:,2));
    report.rmse_allpoints_fit = sqrt(mean((x_all_f - roiHSI(:,1)).^2 + (y_all_f - roiHSI(:,2)).^2));

    [x_all_e, y_all_e] = transformPointsForward(tform_est, roiPOL(:,1), roiPOL(:,2));
    report.rmse_allpoints_est = sqrt(mean((x_all_e - roiHSI(:,1)).^2 + (y_all_e - roiHSI(:,2)).^2));


    % Check corner error (to know whether the transform distorts the 4 manually picked corners)
    [xf_c, yf_c] = transformPointsForward(tform_fit, ptsPOL(:,1), ptsPOL(:,2));
    report.rmse_corners_fit = sqrt(mean((xf_c - ptsHSI(:,1)).^2 + (yf_c - ptsHSI(:,2)).^2));

    [xe_c, ye_c] = transformPointsForward(tform_est, ptsPOL(:,1), ptsPOL(:,2));
    report.rmse_corners_est = sqrt(mean((xe_c - ptsHSI(:,1)).^2 + (ye_c - ptsHSI(:,2)).^2));

    % Check symmetrical/ bidirectional error, ie Forward error: POL to HSI; Backward error: HSI to POL; err_sym = (err_fwd + err_bkw) / 2;
    
    % Forward
    [xf_fwd, yf_fwd] = transformPointsForward(tform_fit, ptsPOL(:,1), ptsPOL(:,2));
    % Backward
    tform_fit_inv = invert(tform_fit);
    [xb_bkw, yb_bkw] = transformPointsForward(tform_fit_inv, ptsHSI(:,1), ptsHSI(:,2));
    report.rmse_symmetric_fit = (sqrt(mean((xf_fwd - ptsHSI(:,1)).^2 + (yf_fwd - ptsHSI(:,2)).^2)) + ... % err_fwd
                                 sqrt(mean((xb_bkw - ptsPOL(:,1)).^2 + (yb_bkw - ptsPOL(:,2)).^2))) / 2; %err_bkw

    % Forward
    [xe_fwd, ye_fwd] = transformPointsForward(tform_est, ptsPOL(:,1), ptsPOL(:,2));
    % Backward
    tform_est_inv = invert(tform_est);
    [xb_bkw_e, yb_bkw_e] = transformPointsForward(tform_est_inv, ptsHSI(:,1), ptsHSI(:,2));
    report.rmse_symmetric_est = (sqrt(mean((xe_fwd - ptsHSI(:,1)).^2 + (ye_fwd - ptsHSI(:,2)).^2)) + ... % err_fwd
                                 sqrt(mean((xb_bkw_e - ptsPOL(:,1)).^2 + (yb_bkw_e - ptsPOL(:,2)).^2))) / 2; %err_bkw
