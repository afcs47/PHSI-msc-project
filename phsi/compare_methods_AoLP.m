...
% Ensure AoLP is in [0, pi)
wrap_pi = @(x) mod(x, pi);
mean_AoLP_standard = wrap_pi(mean_AoLP_standard);
mean_AoLP_1 = wrap_pi(mean_AoLP_1);
mean_AoLP_2 = wrap_pi(mean_AoLP_2);
...
% Compute angular differences for AoLP (respecting AoLP periodicity = pi)
delta1 = mean_AoLP_1 - mean_AoLP_standard;
delta2 = mean_AoLP_2 - mean_AoLP_standard;
deltaf = mean_AoLP_1 - mean_AoLP_2;

% Use doubling/halving trick for period-pi angles:
diff_1 = 0.5 * atan2(sin(2*delta1), cos(2*delta1));
diff_2 = 0.5 * atan2(sin(2*delta2), cos(2*delta2));
diff_fourier = 0.5 * atan2(sin(2*deltaf), cos(2*deltaf));
